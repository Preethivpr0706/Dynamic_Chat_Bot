...
function getClientID(displayPhoneNumber, from) {
  return new Promise((resolve, reject) => {
    ...
    console.log(
      `Fetching Client ID for displayPhoneNumber: "${displayPhoneNumber}"`
    );
    ...
    connection.execute(query, [displayPhoneNumber], async (err, results) => {
      ...
      console.warn(
        `No client found for displayPhoneNumber: "${displayPhoneNumber}"`
      );
...
function getWelcomeMessage(clientId) {
  return new Promise((resolve, reject) => {
    const query =
      'SELECT Value_name FROM List WHERE Client_ID =? AND Key_name = "GREETINGS"';
    const connection = getConnection();
...
function getAvailableDates(iClientId, iMenuId, iKey) {
  return new Promise((resolve, reject) => {
    console.log(
      `getAvailableDates: iClientId:${iClientId} ,iMenuId: ${iMenuId}, iKey: ${iKey}`
    );
    const query = `
            SELECT DISTINCT
                ? AS CLIENT_ID,
...
async function insertAppointment(clientId, userId) {
  const query =
    "INSERT INTO Appointments (Client_ID, User_ID, POC_ID, Appointment_Date, Appointment_Time, Appointment_Type, Status, Is_Active, JSON_DATA) VALUES (?,?,?,?,?,?,?,?,?)";
  const connection = getConnection();
  return new Promise((resolve, reject) => {
    connection.execute(
      query,
